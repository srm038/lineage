...
Lineage uses a modified form of the Multiple-Surnames System with the NGQS numbering system. A `+` next to the name of a child indicates that the line of descent passes through them. Multiple family lines are represented, and so each name in the genealogy is assigned a number in ascending order of birth (for simplicity, only descendants who are carried forward to the root person are assigned a number). These numbers are provided for names which appear elsewhere in the narrative, particularly in the case of wives and in-laws, to facilitate look-up between similar names. The entries are divided by generation, counting down from the earliest known ancestor of any line.
